...
| Package manager | npm (lockfile: `package-lock.json`); `bun.lock` also present |
...
├── push.sh            # Release helper (dev → main merge → npm publish)
...
- `HARDCODED_IGNORES` — always-excluded patterns: `node_modules`, `.git`, `.env`/`.env.*` (but NOT `.env.example`), lock files, build outputs (`dist`, `build`, `out`, `coverage`), logs, OS junk.
- `BINARY_EXTENSIONS` — extension set for binary files that are skipped.
...
- `findFiles(...)` — two-stage filtering:
  ...
  2. Per-file: `.gitignore` check, binary extension check, then async read.
  - Returns `FileContent[]` with **normalized (forward-slash) relative paths**.
  - File reads happen concurrently via `Promise.all`; unreadable files are logged and skipped (never fatal).
...
- Comments in the code explain *why* (e.g., why the output file is added to fast-glob ignores) — preserve that intent when editing.
...
- The `push.sh` script handles releases: checkout `main` → fetch+merge `dev` → push → `npm publish` → back to `dev`. Treat it as an operator script, not part of normal development.
